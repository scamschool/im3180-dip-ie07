...
import styles from './PopUpQuadStyle';
...
const getSheetData = async (filterDay) => {
  try {
    const response = await fetch(
      'https://sheets.googleapis.com/v4/spreadsheets/1dBDgXQRbJYZQTPfNbU1qovM5s6QG_g4XJ_9z4hEB2n0/values/Sheet1!A1:F76?key=AIzaSyBp1JCXECERdbxhx3YeqpFQAd8mM1NLdpk'
    );
...
export default function PopUpQuad({ navigation }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await getSheetData('Saturday'); // Specify the day to filter for
  ...
  <View style={styles.sectionHeader}>
    ...
    <TouchableOpacity
      ...
      onPress={() => {
        ...
        navigation.navigate('QuadMenuScreen'); // Example navigation, change as necessary
      }}
    >
    ...
    {loading ? (
      ...
      <BarChart
        ...
        chartConfig={{
          ...
          backgroundGradientFrom: '#1D3F73',
          backgroundGradientTo: '#1D3F73',
          decimalPlaces: 0,
